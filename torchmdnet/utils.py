...
def write_as_hdf5(files, hdf5_dataset):
    ...
    with h5py.File(hdf5_dataset, "w") as f:
        for i in range(len(files["pos"])):
            ...
            if "ext_pos" in files:
                extcoord_data = np.load(files["ext_pos"][i], mmap_mode="r")
                group.create_dataset("ext_pos", data=extcoord_data)
            if "ext_charge" in files:
                extcharge_data = np.load(files["ext_charge"][i], mmap_mode="r")
                group.create_dataset("ext_charge", data=extcharge_data)
            if "esp" in files:
                esp_data = np.load(files["esp"][i], mmap_mode="r")
                group.create_dataset("esp", data=esp_data)
            if "esp_grad" in files:
                espgrad_data = np.load(files["esp_grad"][i], mmap_mode="r")
                group.create_dataset("esp_grad", data=espgrad_data)
